...
from robot_utils import send_message, template_card_content
...
def load_bot_templates() -> BotTemplates:
    ...
    if not welcome_card_id:
        raise ValueError("WELCOME_CARD_ID is required")
    if not alert_card_id:
        raise ValueError("ALERT_CARD_ID is required")
    if not alert_resolved_card_id:
        raise ValueError("ALERT_RESOLVED_CARD_ID is required")

    return BotTemplates(
        welcome_card_id=welcome_card_id,
        alert_card_id=alert_card_id,
        alert_resolved_card_id=alert_resolved_card_id,
    )
